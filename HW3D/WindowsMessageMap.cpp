...
#define REGISTER_MESSAGE(msg){msg,#msg}//该宏给参数加双引号

WindowsMessageMap::WindowsMessageMap()//内部使用上述宏把所有的"消息符号"全部映射到"字符串"
	:
	map({
...
std::string WindowsMessageMap::operator()(DWORD msg, LPARAM lp, WPARAM wp) const
{
	...
	std::ostringstream oss;//ostringstream类通常用于执行C风格的串流的输出操作，格式化字符串，避免申请大量的缓冲区，替代sprintf。
	if (i != map.end())
	{
